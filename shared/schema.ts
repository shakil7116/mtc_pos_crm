...
export const settings = pgTable("settings", {
  ...
  storeNameAr: text("store_name_ar").notNull().default(""),
  ...
  // Discounts OVER this need a manager PIN; at or under it a salesman just gives it.
  // A walk-in customer will not stand at the counter waiting for an approval, so a
  // blocking gate on every small discount costs real sales. Small ones flow, big
  // ones stop — and every discount is recorded against whoever gave it either way.
  discountApprovalThreshold: numeric("discount_approval_threshold").default("100"),
