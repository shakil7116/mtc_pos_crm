...
import {
  ...
  ArrowRight, User, CalendarDays, Zap, MessageSquare, ChevronRight,
} from "lucide-react";
...
/** Max cards rendered inline per kanban column — the rest live behind "See all". */
...
function TaskCard({ t, onClick }: { t: any; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
    ...
    >
      ...
      <div className="mt-2 flex items-center justify-between gap-2 text-[11px] text-slate-400 dark:text-slate-500">
        <span className="shrink-0">{fmtDate(t.dueDate || t.createdAt)}</span>
        <span className="truncate text-slate-500 dark:text-slate-400">{t.assignedToName}</span>
      </div>
    </div>
  );
}
...
export default function TasksPanel() {
  ...
  const [viewAllCol, setViewAllCol] = useState<string | null>(null);
  ...
  return (
    <section className="rounded-2xl border bg-white border-slate-200 shadow-sm dark:bg-slate-900/60 dark:border-slate-800 dark:shadow-lg dark:shadow-black/20 p-5">
            ...
            <div key={col.key} className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-800/20">
              ...
              <div className="space-y-2">
                {list.length === 0 ? (
                  ...
                  list.slice(0, COLUMN_PREVIEW).map((t) => (
                    <TaskCard key={t.id} t={t} onClick={() => setSelectedTask(t)} />
                  ))
                )}
              </div>
              {list.length > COLUMN_PREVIEW && (
                <button
                  onClick={() => setViewAllCol(col.key)}
                  className="mt-2 flex w-full items-center justify-center gap-1 rounded-lg border border-dashed border-slate-300 py-2 text-xs font-semibold text-slate-500 transition-colors hover:border-slate-400 hover:bg-white hover:text-slate-700 dark:border-slate-700 dark:text-slate-400 dark:hover:border-slate-600 dark:hover:bg-slate-900/60 dark:hover:text-slate-200"
                >
                  See all {list.length}
      ...
      {/* ── See-all column dialog ── */}
      <Dialog open={!!viewAllCol} onOpenChange={(v) => { if (!v) setViewAllCol(null); }}>
        <DialogContent className="max-w-lg">
          {(() => {
            const col = KANBAN_COLS.find((c) => c.key === viewAllCol);
            const list = viewAllCol ? (kanban[viewAllCol] ?? []) : [];
            return (
              <>
                <DialogHeader>
                  <DialogTitle className="flex items-center gap-2 text-base">
                    <span className={`h-2 w-2 rounded-full ${col?.dot ?? "bg-slate-400"}`} />
                    {col?.label ?? "Tasks"}
                ...
                <div className="-mr-2 max-h-[60vh] space-y-2 overflow-y-auto pr-2">
                  {list.map((t) => (
                    <TaskCard
                      key={t.id}
                      t={t}
                      onClick={() => { setViewAllCol(null); setSelectedTask(t); }}
                    />
                  ))}
                </div>
                <DialogFooter>
                  <Button variant="outline" onClick={() => setViewAllCol(null)}>Close</Button>
                </DialogFooter>
              </>
            );
          })()}
