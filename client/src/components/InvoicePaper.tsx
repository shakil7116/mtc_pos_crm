...
export const InvoicePaper = forwardRef<HTMLDivElement, InvoicePaperProps>(
  ({ settings, invoice, className, template = "template1", docType: docTypeProp }, ref) => {
    ...
    // Title labels
    const docTitleEn = isDeliveryNote ? "DELIVERY NOTE" : isQuotation ? "QUOTATION" : isCreditNote ? "CREDIT NOTE" : isReturn ? "RETURN INVOICE" : "INVOICE";
    const docTitleAr = isDeliveryNote ? "إشعار تسليم" : isQuotation ? "عرض سعر" : isCreditNote ? "إشعار دائن" : isReturn ? "فاتورة إرجاع" : "فاتورة";
